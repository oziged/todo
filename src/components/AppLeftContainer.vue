<template>
  <div class="app__left-container">
    <div 
      :class="{
        'lists-btn': true,
        'btn-default': true,
        'btn-default_has_icon': true,
        'btn-hover': $store.state.mode != 'all',
        'btn-active': $store.state.mode === 'all'  
      }"
      @click="showAllLists"
    >
    <div class="btn-default__left-icon">
      <svg
        class="lists-btn__icon " 
        width="14" 
        height="12" 
        viewBox="0 0 14 12" 
        fill="none" 
...
<script>
import TodoListGroupBtn from '@/components/TodoListGroupBtn'
import AddList from '@/components/AddList'

export default {
  components: {TodoListGroupBtn, AddList},
  ...
  methods: {
    showAllLists() {
      this.$store.commit('SET_STATE', {key: 'mode', value: 'all'})
    }
  }
